...
const TRAINER_STATS_PASSWORD_HASH =
  'b2294cbadf6f4da2e844eba86d816356723b559073f422547246c13f4a3c4d87';
...
if (typeof CONFIG === 'undefined') {
  console.error(
    'CONFIG not found! Make sure config.js is loaded before this script.'
  );
...
const LOCAL_API_BASE =
  (CONFIG.LOCAL_PROXY || 'http://localhost:3000') + '/api/conversations';
const TURING_API_BASE =
  CONFIG.API_BASE_URL || 'https://labeling-g.turing.com/api/conversations';
...
const ROLE_INFO = {
  'code-trainer': {
    label: 'Code Trainer',
    waitingLabel: 'Waiting for Code Trainer',
    icon: '💻',
    color: '#3b82f6',
  },
  'code-reviewer': {
    label: 'Code Reviewer',
    waitingLabel: 'Waiting for Code Reviewer',
    icon: '🔍',
    color: '#8b5cf6',
  },
  'stem-tasker': {
    label: 'STEM Tasker',
    waitingLabel: 'Waiting for STEM Tasker',
    icon: '🔬',
    color: '#10b981',
  },
  'stem-reviewer': {
    label: 'STEM Reviewer',
    waitingLabel: 'Waiting for STEM Reviewer',
    icon: '📋',
    color: '#f59e0b',
  },
  unknown: {
...
function calculateDetailedBreakdown(tasks, tabName) {
  ...
  tasks.forEach((task) => {
    ...
    const taskInfo = {
      ...
      link:
        task.colabLink ||
        `https://labeling-g.turing.com/conversations/${task.id}`,
      formStage: formStage,
    };
    ...
    if (!breakdown.bySubject[subject].byFormStage[formStage]) {
      breakdown.bySubject[subject].byFormStage[formStage] = {
        total: 0,
        byRole: {},
        tasks: [],
      };
...
function updateSidebar(breakdown, tabName) {
  ...
  const sortedSubjects = KNOWN_SUBJECTS.filter(
    (s) => breakdown.bySubject[s]
  ).concat(
    Object.keys(breakdown.bySubject).filter((s) => !KNOWN_SUBJECTS.includes(s))
  );
    ...
    for (const role of roles) {
      ...
      if (roleData && roleData.count > 0) {
        ...
        if (roleData.tasks.length > 15) {
          html += `<span class="more-tasks-sm">+${
            roleData.tasks.length - 15
          } more</span>`;
        }
...
async function loadSubjectWithStatusTabs(subject) {
  const statusTabsContainer = document.getElementById(
    'subject-status-tabs-container'
  );
  ...
  try {
    ...
    allSubjectTasks.forEach((statusData) => {
      const statusInfo = statusDisplayNames[statusData.status] || {
      ...
      contentHTML += `
                <div class="subject-status-section ${collapsedClass}" id="subject-status-${statusId}">
                    <div class="status-section-header">
                        <h3><span class="status-icon">${
                          statusInfo.icon
                        }</span> ${statusInfo.label}</h3>
    ...
    contentContainer.innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">⚠️</div><div class="empty-state-text">Error: ' +
      error.message +
      '</div></div>';
  }
}
...
function displaySubjectStatusTasks(statusId, statusName, tasks) {
  ...
  if (tasks.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">📭</div><div class="empty-state-text">No tasks</div></div>';
  ...
  if (summaryContainer) {
    summaryContainer.innerHTML =
      '<div class="subject-task-count">Total: <strong>' +
      tasks.length +
      '</strong> task' +
      (tasks.length !== 1 ? 's' : '') +
      '</div>';
  ...
  const roleDisplayNames = {
    'code-trainer': { label: 'CODE Trainer', icon: '💻' },
    'code-reviewer': { label: 'CODE QC', icon: '💻' },
    'stem-tasker': { label: 'STEM Tasker', icon: '🔬' },
    'stem-reviewer': { label: 'STEM QC', icon: '🔬' },
  };
  ...
  // Display tasks grouped by phase, then by role
  let html = '';
  ...
  Object.keys(tasksByPhase)
    ...
    .forEach(function (phase) {
      ...
      // Group tasks within phase by specific role responsibility
      const tasksByRole = {};
      ...
      // Display tasks grouped by specific role
      const roleOrder = [
        'code-trainer',
        'code-reviewer',
        'stem-tasker',
      ...
      roleOrder.forEach(function (role) {
        if (tasksByRole[role] && tasksByRole[role].length > 0) {
          const roleInfo = roleDisplayNames[role];
          const roleCount = tasksByRole[role].length;

          html += `
                    <div class="team-subsection">
                        <div class="team-label">${roleInfo.icon} ${roleInfo.label} (${roleCount})</div>
                        <div class="team-task-list">
                `;

          tasksByRole[role].forEach(function (task) {
            const taskId = String(task.id);
            const link = task.colabLink || '#';
            html += `<a href="${link}" target="_blank" title="Task ${taskId}" class="task-chip">${taskId}</a>`;
          });
...
function displayTasks(tabName, tasks, summaryContainer, container) {
  ...
  if (tasks.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">📭</div><div class="empty-state-text">No tasks found</div></div>';
  ...
  if (tabName === 'delivery') {
    ...
    for (const [subject, deliveryBatches] of Object.entries(grouped)) {
      const totalCount = Object.values(deliveryBatches).reduce(
        (sum, count) => sum + count,
        0
      );
  ...
  if (tabName === 'trainer-stats') {
    ...
    const sortedOverallFormStages = Object.entries(overallFormStages).sort(
      (a, b) => b[1].count - a[1].count
    );
    for (const [formStage, fsData] of sortedOverallFormStages) {
      const fsAht =
        fsData.count > 0 ? Math.round(fsData.minutes / fsData.count) : 0;
      const colorClass = getFormStageColorClass(formStage);
      formStageAhtHtml += `
                <div class="summary-pill formstage-aht-pill ${colorClass}" title="${formStage}: ${
        fsData.count
      } tasks, ${formatMinutes(fsData.minutes)} total">
      ...
      summaryContainer.innerHTML = `
                <div class="summary-row summary-main">
                    ...
                    <div class="summary-pill trainer-time">
                        ...
                        <span class="pill-count">${formatMinutes(
                          totalMinutes
                        )}</span>
                    </div>
    ...
    const sortedTrainers = Object.entries(trainerStats).sort(
      (a, b) => b[1].taskCount - a[1].taskCount
    );
    ...
    for (const [trainerName, stats] of sortedTrainers) {
      const aht =
        stats.taskCount > 0
          ? Math.round(stats.totalMinutes / stats.taskCount)
          : 0;
      ...
      html += `<div class="subject-card trainer-card">
                    ...
                    <div class="trainer-info">
                        ${
                          stats.profilePicture
                            ? `<img src="${stats.profilePicture}" class="trainer-avatar" alt="${trainerName}">`
                            : '<div class="trainer-avatar-placeholder">👤</div>'
                        }
                ...
                <div class="trainer-status-row">
                    <div class="status-badge status-reviewed" title="Reviewed: Completed tasks awaiting delivery">📋 ${
                      stats.reviewed
                    }</div>
                    <div class="status-badge status-indelivery" title="In Delivery: Tasks in ongoing delivery batch">📦 ${
                      stats.inDelivery
                    }</div>
                    <div class="status-badge status-delivered" title="Delivered: Tasks successfully delivered to client">✅ ${
                      stats.delivered
                    }</div>
                </div>
                <div class="trainer-stats-grid">
                    <div class="trainer-stat-card stat-time">
                        ...
                        <div class="stat-value">${formatMinutes(
                          stats.totalMinutes
                        )}</div>
                    </div>
      ...
      const sortedFormStages = Object.entries(stats.formStages).sort(
        (a, b) => b[1].count - a[1].count
      );

      for (const [formStage, fsStats] of sortedFormStages) {
        const fsAht =
          fsStats.count > 0 ? Math.round(fsStats.minutes / fsStats.count) : 0;
        const colorClass = getFormStageColorClass(formStage);
        html += `<div class="trainer-formstage-item ${colorClass}">
                    <span class="trainer-formstage-name">${formStage}</span>
                    <span class="trainer-formstage-count">${
                      fsStats.count
                    }</span>
                    <span class="trainer-formstage-time">${formatMinutes(
                      fsStats.minutes
                    )}</span>
      ...
      const sortedSubjects = Object.entries(stats.subjects).sort(
        (a, b) => b[1].count - a[1].count
      );

      for (const [subject, subjectStats] of sortedSubjects) {
        const subjectAht =
          subjectStats.count > 0
            ? Math.round(subjectStats.minutes / subjectStats.count)
            : 0;
        const deliveredBadge =
          subjectStats.delivered > 0
            ? `<span class="delivered-indicator" title="${subjectStats.delivered} tasks delivered">✅${subjectStats.delivered}</span>`
            : '';
        html += `<div class="trainer-subject-item">
                    <span class="trainer-subject-name">${subject}</span>
                    <span class="trainer-subject-count">${
                      subjectStats.count
                    } ${deliveredBadge}</span>
                    <span class="trainer-subject-time">${formatMinutes(
                      subjectStats.minutes
                    )}</span>
  ...
  for (const formStages of Object.values(grouped)) {
    for (const [formStage, count] of Object.entries(formStages)) {
      overallFormStageTotals[formStage] =
        (overallFormStageTotals[formStage] || 0) + count;
      grandTotal += count;
    }
  }
  ...
  if (summaryContainer) {
    ...
    for (const [formStage, count] of Object.entries(overallFormStageTotals)) {
      const colorClass =
        tabName === 'improper' ? 'improper' : getFormStageColorClass(formStage);
  ...
  for (const [subject, formStages] of Object.entries(grouped)) {
    const totalCount = Object.values(formStages).reduce(
      (sum, count) => sum + count,
      0
    );
    ...
    html += `<div class="subject-card">
            ...
            <div class="formstage-grid ${
              formStageEntries.length === 3
                ? 'three-cards'
                : formStageEntries.length === 4
                ? 'four-cards'
                : ''
            }">`;
...
function groupTasksBySubjectAndDeliveryBatch(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    if (task.latestDeliveryBatch && task.latestDeliveryBatch.deliveryBatch) {
      batchName =
        task.latestDeliveryBatch.deliveryBatch.name || 'Unknown Batch';
...
function normalizeSubject(subject) {
  ...
  if (subjectLower === 'data science' || subjectLower === 'datascience')
    return 'Data Science';
...
function normalizeFormStage(formStage) {
  ...
  if (trimmed.toLowerCase().includes('ground truth'))
    return 'Ground Truth and ICE';
  if (
    trimmed.toLowerCase().includes('image rubrics') ||
    trimmed.toLowerCase().includes('gemini')
  )
    return 'Image Rubrics and Gemini';
...
function getFormStageColorClass(formStage) {
  ...
  if (lower.includes('image rubrics') || lower.includes('gemini'))
    return 'image-rubrics';
  return 'no-formstage';
}
...
function groupTasksByTrainer(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    trainers[trainerName].subjects[subject].formStages[formStage].minutes +=
      durationMinutes;
  });
...
function filterTrainers() {
  const searchTerm =
    document.getElementById('trainer-search')?.value?.toLowerCase() || '';
  const sortValue =
    document.getElementById('trainer-sort')?.value || 'tasks-desc';
  ...
  switch (sortValue) {
    ...
    case 'aht-desc':
      return entries.sort((a, b) => {
        const ahtA =
          a[1].taskCount > 0 ? a[1].totalMinutes / a[1].taskCount : 0;
        const ahtB =
          b[1].taskCount > 0 ? b[1].totalMinutes / b[1].taskCount : 0;
        return ahtB - ahtA;
      });
    case 'aht-asc':
      return entries.sort((a, b) => {
...
function renderTrainerCards(trainersArray) {
  ...
  if (trainersArray.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">🔍</div><div class="empty-state-text">No trainers found</div></div>';
    return;
  }
